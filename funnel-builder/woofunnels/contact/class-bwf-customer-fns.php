...
/**
 * Customer aggregate (totals, products, terms) maintenance
 *
 * Implementations behind the bwf_* functions in woofunnels-customer-functions.php —
 * the function signatures there are frozen API and delegate here, so this
 * file parses only when one of them is first called.
 *
 * @package WooFunnels
 */
if ( ! class_exists( 'BWF_Customer_Fns' ) ) {

	class BWF_Customer_Fns {

		public static function bwf_create_update_customer( $bwf_contact, $order, $order_id, $products, $total ) {
		...
		public static function bwf_update_cats_and_tags( $product_id, $cat_ids, $tag_ids ) {
		...
		public static function bwf_update_customer_refunded( $order_id, $refund_amount ) {
		...
		public static function bwf_reduce_customer_total_on_cancel( $order_id ) {
			$order = wc_get_order( $order_id );
		...
		public static function bwf_get_order_total( $order ) {
			$total = 0;
		...
		public static function bwf_get_order_product_ids( $order ) {
			$product_ids = [];
		...
		public static function bwf_get_order_product_terms( $order ) {
			$product_ids = [];
		...
		public static function bwf_get_array_unique_integers( $array1 = [], $array2 = [] ) {
