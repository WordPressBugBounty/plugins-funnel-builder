...
if ( ! class_exists( 'WFFN_Step_WC_Checkout' ) ) {
	...
	class WFFN_Step_WC_Checkout extends WFFN_Step {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_after_add_to_cart_form', array( $this, 'fk_checkout_redirect_to_dedicated_page' ) );
		}
		...
		/**
		 * Proxy: lazy-load FK_Checkout_Redirect and output redirect JS on single product pages.
		 */
		public function fk_checkout_redirect_to_dedicated_page() {
			if ( ! is_product() ) {
				return;
			}
			require_once WFFN_PLUGIN_DIR . '/includes/class-fk-checkout-redirect.php';

			FK_Checkout_Redirect::get_instance()->maybe_redirect_add_to_cart();
		}
	}
