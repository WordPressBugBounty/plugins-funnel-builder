...
if ( ! class_exists( 'WFOP_Divi_Field' ) ) {
	...
	abstract class WFOP_Divi_Field extends ET_Builder_Module {
		...
		protected function add_typography( $tab_id, $key, $selectors = '', $label = '', $default = '#333333', $conditions = array(), $font_side_default = array() ) {
		...
		protected function add_font( $tab_id, $key, $selectors = '', $label = '', $default = '#000000', $conditions = array() ) {
			if ( empty( $label ) ) {
		...
		protected function add_border( $tab_id, $key, $selectors, $conditions = array(), $default = array(), $fields_options = array() ) { //phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedParameter,VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		protected function add_box_shadow( $tab_id, $key, $selectors, $label = '', $default = array(), $conditions = array() ) {
		...
		protected function add_width( $tab_id, $key, $selectors = '', $label = '', $default = array(), $conditions = array(), $responsive = false ) {
		...
		protected function add_max_width( $tab_id, $key, $selectors = '', $label = '', $default = array(), $conditions = array(), $responsive = false ) {
		...
		public function render_ajax() {
			if ( ! check_ajax_referer( 'wfop_divi_ajax', '_ajax_nonce', false ) ) {
			...
			$this->props = $_REQUEST; //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce already verified above
			echo "<div id='" . esc_attr( $this->slug ) . "'>" . $this->html( array() ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $this->html() returns safe HTML for Divi builder rendering
			exit;
		}
