...
if ( ! class_exists( 'Elementor_WFFN_Optin_Form_Widget' ) ) {
	...
	class Elementor_WFFN_Optin_Form_Widget extends \Elementor\Widget_Base {
		...
		/**
		 * The optin form renders against live request state and must never have its markup baked
		...
		public function register_sections() {
			...
			$this->add_text( 'button_text', __( 'Title', 'funnel-builder' ), __( 'Send Me My Free Guide', 'funnel-builder' ), array(), '', '', __( 'Enter the Button Text', 'funnel-builder' ) );
			$this->add_text( 'subtitle', 'Sub Title', '', array(), '', '', __( 'Enter subtitle', 'funnel-builder' ) );
		...
		public function register_styles() {
			...
			$this->add_text_alignments( 'button_alignment', array( '{{WRAPPER}} .wffn-custom-optin-from #bwf-custom-button-wrap' ), __( 'Alignment', 'funnel-builder' ) );
			$this->add_text_alignments( 'button_text_alignment', array( '{{WRAPPER}} .wffn-custom-optin-from #bwf-custom-button-wrap span' ), __( 'Text Alignment', 'funnel-builder' ) );
			...
			$this->add_background_color( 'button_bg_color', array( '{{WRAPPER}} .bwfac_form_sec #wffn_custom_optin_submit' ), '#FBA506' );
			$this->add_color( 'button_color', array( '{{WRAPPER}} .bwfac_form_sec #wffn_custom_optin_submit .bwf_heading, {{WRAPPER}} .bwfac_form_sec #wffn_custom_optin_submit .bwf_subheading' ), '#ffffff' );
			$this->close_controls_tab();
			$this->add_controls_tab( 'bwf_button_hover_tab', 'Hover' );
			$this->add_background_color( 'button_hover_bg_color', array( '{{WRAPPER}} .bwfac_form_sec #wffn_custom_optin_submit:hover' ), '#E69500' );
			$this->add_color(
				'button_hover_color',
				array(
		...
		protected function render() {
			...
			/**
			 * The per-field width controls are only registered when register_sections() runs
			 ...
			 * Fall back to the field's own width, and then to the same default
			 * register_sections() uses, instead of reading an undefined key.
			 */
		...
		protected function add_width( $field_key, $selector, $label = '', $default = array(), $condition = array(), $size_unit = array(), $tablet_default = array(), $mobile_default = array() ) {
			if ( empty( $label ) ) {
				$label = __( 'Width', 'elementor' );
			}
