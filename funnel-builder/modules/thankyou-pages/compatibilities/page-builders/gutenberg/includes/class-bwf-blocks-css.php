  ...
  class BWF_Blocks_CSS {
		...
		public function add_property( $property, $value = null, $unit_value_pair = false ) {
			...
			if ( $unit_value_pair ) {
				...
				if ( in_array( $property, $this->_special_properties_list, true ) ) {
			...
			if ( is_string( $value ) || is_numeric( $value ) ) {
				...
				foreach ( $value as $key => $bg ) {
					...
					switch ( $key ) {
						...
						case 'image' :
							$image_url = $bg['url'] ?? '';
							$image_url && $this->add_rule( "background-$key", sprintf( "url('%s')", $image_url ) );
							break;
		...
		public function add_typograpghy_property( $property, $value, $skipAttrs = [] ) {
			if ( is_array( $value ) ) {
				foreach ( $value as $key => $typo ) {
					if ( 'sizeUnit' === $key || in_array( $key, $skipAttrs, true ) ) {
						continue;
					}
					if ( 'size' === $key ) {
						if ( empty( $typo ) ) {
