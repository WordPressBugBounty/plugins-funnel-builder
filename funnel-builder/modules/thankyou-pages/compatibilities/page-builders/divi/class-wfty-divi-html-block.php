...
if ( ! class_exists( 'WFTY_Divi_HTML_BLOCK' ) ) {
	...
	abstract class WFTY_Divi_HTML_BLOCK extends WFTY_Divi_Field {
		...
		final public function render( $attrs, $content = null, $render_slug = '' ) {
			...
			if ( apply_filters( 'wfty_print_divi_widget', true, $this->get_slug(), $this ) ) {

				return "<div id='{$this->slug}'>" . $this->html() . '</div>';
			}
		...
		protected function available_html_block() {
			...
			return apply_filters( 'wfty_html_block_elements', $block );
		}
		...
		public function localize_array() {
			...
			foreach ( $fields as $key => $field ) {
				if ( isset( $field['c_type'] ) && 'wfty_start_border' === $field['c_type'] ) {
					$border_start                       = true;
					$border_data[ $field['field_key'] ] = $field['selector'];
					continue;
				}
				if ( isset( $field['c_type'] ) && 'wfty_end_border' === $field['c_type'] ) {
					$border_start = false;
					continue;
				}
				if ( isset( $field['c_type'] ) && 'wfty_start_box_shadow' === $field['c_type'] ) {
					$box_start                       = true;
					$box_data[ $field['field_key'] ] = $field['selector'];
					continue;
				}
				if ( isset( $field['c_type'] ) && 'wfty_end_box_shadow' === $field['c_type'] ) {
					$box_start = false;
					continue;
				}
		...
		public function prepare_css( $attrs, $content, $render_slug ) {
			...
			foreach ( $fields as $key => $field ) {
				...
				if ( false !== strpos( $key, '_margin' ) || false !== strpos( $key, '_padding' ) ) {
					...
					if ( $key === 'wfty_form_fields_focus_color' ) {
						ET_Builder_Element::set_style( $render_slug, array(
							'selector'    => $field['selector'],
