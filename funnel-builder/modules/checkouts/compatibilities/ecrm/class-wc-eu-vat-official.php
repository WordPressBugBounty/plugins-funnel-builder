...
if ( ! class_exists( 'WFACP_Compatibility_With_WC_EU_Vat_Official' ) ) {
	/**
	...
	class WFACP_Compatibility_With_WC_EU_Vat_Official {
		...
		public function __construct() {
			add_filter( 'wfacp_advanced_fields', array( $this, 'add_eu_fields' ) );
		...
		public function add_eu_fields( $field ) {
			...
			if ( $this->is_enable() ) {
				$field['wc_eu_vat_official_vat_number'] = array(
					'type'        => 'wfacp_html',
					'field_type'  => 'advanced',
					'class'       => array( 'wfacp_wc_eu_vat_official_vat_number' ),
					'id'          => 'wc_eu_vat_official_vat_number',
					'label'       => __( 'EU VAT', 'woocommerce' ), //phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					'placeholder' => __( 'EU VAT', 'woocommerce' ), //phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				);
			}
			...
			<script>
				jQuery(function ($) {
					setTimeout(function () {
						...
						var billingWrap = '#woocommerce_eu_vat_number_field';
						var shippingWrap = '#woocommerce_eu_vat_number_shipping_field';
						var last = {};

						$(formSel).off('change', billingWrap).off('change', shippingWrap);

						$(formSel).on('change', billingWrap + ', ' + shippingWrap, function () {
							var $input = $(this).find('input').first();
							if (!$input.length) return;
							var id = $input.attr('id') || 'vat';
