...
if ( ! class_exists( 'BWF_Admin_General_Settings' ) ) {
	...
	class BWF_Admin_General_Settings {
		...
		public function __construct() {
			...
			add_filter(
				...
				function ( $menu ) {
					array_push(
						$menu,
						array(
							'title' => __( 'General', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			...
			add_action( 'wp_ajax_bwf_general_settings_update', array( $this, 'update_general_settings' ) );
		...
		public function __callback() {
			...
			if ( class_exists( 'BWF_Admin_Breadcrumbs' ) ) {
				BWF_Admin_Breadcrumbs::register_node( array( 'text' => 'Settings' ) );
			}
			BWF_Admin_Breadcrumbs::render_sticky_bar();
			?>
			<div class="wrap bwf-funnel-common">
				<h1 class="wp-heading-inline"><?php esc_html_e( 'Settings', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?></h1>
				<?php
				...
				<div id="bwf_general_settings_vue_wrap" class="bwf-hide" v-bind:class="`1`===is_initialized?'bwf-show':''">
					...
					<div class="bwf-tabs-view-vertical bwf-widget-tabs">
						<div class="bwf-tabs-wrapper">
							<div class="bwf-tab-title" data-tab="
							<?php
							++$i;
							echo esc_attr( $i );
							?>
							" role="tab">
								<?php esc_html_e( 'Permalinks', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
							</div>
							<div class="bwf-tab-title" data-tab="
							<?php
							++$i;
							...
							?>
							" role="tab">
								<?php esc_html_e( 'Facebook Pixel', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
							</div>
							<div class="bwf-tab-title" data-tab="
							<?php
							++$i;
							...
							?>
							" role="tab">
								<?php esc_html_e( 'Google Analytics', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
							</div>
							...
							<?php if ( apply_filters( 'bwf_enable_ecommerce_integration_gad', false ) ) { ?>
								...
								?>
								" role="tab">
									<?php esc_html_e( 'Google Ads', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
								</div>
								<?php
							}
							if ( apply_filters( 'bwf_enable_ecommerce_integration_pinterest', false ) ) {
								...
								?>
								" role="tab">
									<?php esc_html_e( 'Pinterest', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
								</div>
								<?php
							}
							...
							if ( apply_filters( 'bwf_enable_ecommerce_integration_tiktok', false ) ) {
								...
								?>
								" role="tab">
									<?php esc_html_e( 'TikTok', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
								</div>
								<?php
							}
							if ( apply_filters( 'bwf_enable_ecommerce_integration_snapchat', false ) ) {
								...
								?>
								" role="tab">
									<?php esc_html_e( 'Snapchat', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?>
								</div>
							<?php } ?>
						...
						<div class="bwf-tabs-content-wrapper">
							<div class="bwf_setting_inner">
								...
								<div class="bwf_form_button">
									...
									<button v-on:click.self="onSubmit" class="bwf_save_btn_style"><?php esc_html_e( 'Save Changes', 'funnel-builder' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch ?></button>
		...
		public function default_general_settings() {
			...
			return apply_filters(
				...
				array(
					...
					'track_traffic_source'                => array(), // for backcompat, not calling anywhere
					'ga_track_traffic_source'             => array(), // for backcompat, not calling anywhere
		...
		public function maybe_add_js() {
			wp_enqueue_script( 'bwf-general-settings', plugin_dir_url( WooFunnel_Loader::$ultimate_path ) . 'woofunnels/assets/js/bwf-general-settings.js', array(), BWF_VERSION );
		...
		public function get_localized_data() {
			$localized_data = array(
				...
				'texts'                  => array(
					'settings_success'    => __( 'Changes saved', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					'permalink_help_text' => __( 'Leave empty to remove slug completely from url', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				),
				'globalOptionsFields'    => array(
					'options'       => $this->filter_admin_options( $this->get_option() ),
					'legends_texts' => array(
						'fb'         => __( 'Facebook Pixel', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'ga'         => __( 'Google Analytics', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'gad'        => __( 'Google Ads', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'pint'       => __( 'Pinterest', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'permalinks' => __( 'Permalinks', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'tiktok'     => __( 'Tiktok', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
						'snapchat'   => __( 'Snapchat', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					),
			...
			$localized_data['errors']              = array(
				'checkout_slug' => sprintf( __( 'Error: The permalink "%s" is reserved by Native WooCommerce Checkout Page. Try another permalink.', 'funnel-builder' ), $checkout_page_slug ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.WP.I18n.MissingTranslatorsComment
				'empty_base'    => sprintf( __( 'Error: The current Permalinks settings does not allow blank values. Switch Permalink settings to \'Post name\'. <a href="%s">Click Here To Change</a>', 'funnel-builder' ), admin_url( 'options-permalink.php' ) ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.WP.I18n.MissingTranslatorsComment
			);
		...
		public function get_localized_bwf_data() {
			...
			/** Populated by legacy-license-module.php (excluded from the free build); empty array otherwise */
			$localized_data['pro_status'] = apply_filters( 'bwf_general_settings_pro_status', array() );
		...
		public function update_general_settings() {
			if ( ! current_user_can( 'manage_woocommerce' ) ) {
				wp_send_json(
					array(
						'status' => false,
						'msg'    => __( 'You do not have permission to perform this action.', 'funnel-builder' ), // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
					'data'       => '',
					),
					403
				);
		...
		public function update_global_settings_fields( $options ) {
			...
			$resp    = array(
				'status' => false,
				'msg'    => __( 'Settings Updated', 'funnel-builder' ),  // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
				'data'   => '',
			);
		...
		public function maybe_change_title( $title ) {
			if ( 'bwf_settings' === filter_input( INPUT_GET, 'tab', FILTER_UNSAFE_RAW ) || 'bwf_settings' === filter_input( INPUT_GET, 'section', FILTER_UNSAFE_RAW ) ) {
				...
				$title       = sprintf( __( '%1$s &lsaquo; %2$s &#8212; WordPress', 'funnel-builder' ), 'FunnelKit', $admin_title ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch, WordPress.WP.I18n.MissingTranslatorsComment
			}
		...
		public function all_fields() {
			...
			// settings.php lives beside this class in subcore/ now (moved out of the shared
			// woofunnels core); read it __DIR__-local so it resolves no matter which core
			// set $ultimate_path.
			$static_config  = include __DIR__ . '/settings.php';
			$legacy         = apply_filters( 'bwf_general_settings_fields', array() );
			$legacy_altered = array();
			if ( count( $legacy ) > 0 ) {
				$i = 0;
